<ul>
	...
	<li id="dashboard-link" <?php echo 'class="' . (!$this->module > '' ? ' active' : '').'"'?> ><?php echo anchor('admin', '<span class="glyph cloud"></span>'.lang('global:dashboard'), 'class="' . (!$this->module > '' ? ' current' : '').'"');?></li>
		...
		foreach ($menu_items as $menu_item)
		{
			...
			if ($count > 0)
			{
				...
				if ($count > 1 )
				{
					...
					echo anchor(current_url() . '#', '<span class="glyph list"></span>'.$name, array('class' => $class));
		...
		<?php if (array_key_exists('settings', $this->permissions) OR $this->current_user->group == 'admin'): ?>
			<li><?php echo anchor('admin/settings', '<span class="glyph archive"></span>'.lang('cp_nav_settings'), 'class="top-link no-submenu' . (($this->module == 'settings') ? ' current"' : '"'));?></li>
		<?php endif; ?>

		<?php if (array_key_exists('modules', $this->permissions) OR $this->current_user->group == 'admin'): ?>
			<li><?php echo anchor('admin/modules', '<span class="glyph rating"></span>'.lang('cp_nav_addons'), 'class="top-link no-submenu' . (($this->module == 'modules') ? ' current"' : '"'));?></li>
		<?php endif; ?>
		...
		*/
		...
		<li>
			<a href="<?php echo current_url().'#'; ?>"><?php echo '<span class="glyph logout"></span>'.lang('global:profile'); ?></a>
			<ul class="submenu">
