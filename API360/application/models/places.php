...
class PLACES extends CONTROLLERS
{
    ...
    public function GET_PHOTO(){
        ...
        $result = @mysql_query("SELECT id as photo_id , user_id as photo_user , user_id as photo_user_image , place_image as photo_url,  place_thumb, place_info as photo_desecription, place_time_stamp as photo_time_stamp, user_id as photo_user_id  FROM `places_images` WHERE place_id = '{$place_id}'");
        ...
        while ($row = mysql_fetch_assoc($result)) {
            ...
            foreach ($row as $col => $val) {
                ...
                if($col == "photo_user_image"){
                    $get_uid = @mysql_result(mysql_query("SELECT avatar FROM `members` WHERE `id` = '{$val}'"),0 ) or FUNCTIONS::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);
                    $val = $get_uid;
                }
...
AND tour_requests.`approved` = '1'
    ...
    public function ADD_PLACE_IMG(){
        ...
        if($place_thumb == '1'){
            ...
            (`id`,`place_id` ,`place_thumb` ,`place_info` ,`user_id`) VALUES
    ...
    public function TRASH_PLACE(){

        ## account "ID"
        $get_uid = trim($_POST['uid']);

        $place_id = trim($_POST['pid']);

        $result = @mysql_query("UPDATE `tour_requests` SET `trash` = '1' WHERE `id` = '{$place_id}' and `account_id` = '{$get_uid}'");
