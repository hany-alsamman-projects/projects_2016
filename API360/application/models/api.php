...
class API extends CONTROLLERS
{
    ...
    public function RESET_PASSWORD(){
        ...
        $get_uid = @mysql_result(mysql_query("SELECT id FROM `members` WHERE `id` = '{$user_email}' and `activated` = '1'"),0 ) or FUNCTIONS::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);

        if($get_uid){

            $reset_password = trim($_POST['new_password']);

            // Change the password in DB
            mysql_query("UPDATE `members` SET `password` = MD5('$reset_password') WHERE `id` = '{$get_uid}'") or parent::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);
    ...
    public function DELETE_PLACE(){
	    ...
	    $place_name = $_POST['pid'];


        $result = @mysql_query("DELETE FROM `tour_requests` WHERE `id` = '{$place_name}' and `account_id` = '{$get_uid}'") or FUNCTIONS::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);
    ...
    public function TOUR_PLACES(){
        ...
        $result = @mysql_query("SELECT id as place_id , place_name, place_image, place_link, latitude, longitude, place_available, active_date as place_start_date, days_left as end_date FROM `tour_requests` WHERE `account_id` = '{$get_uid}'");
    ...
    public function DAYS_LEFT(){
        ...
        ## account "ID"
        $get_uid = trim($_REQUEST['uid']);

        $result = @mysql_query("SELECT id as place_id, place_name, place_link, place_image, days_left FROM `tour_requests` WHERE `account_id` = '{$get_uid}'");
    ...
    public function SIGNUP_PROCESS()
    {
        ...
        (`id` ,`name` ,`password` ,`last_name` ,`phone_number` ,`gmap` ,`email` , `active_key`, `city`, `dept_id`, `avatar`) VALUES
        (NULL , '{$first_name}', MD5( '{$user_pass}' ) , '{$last_name}' , '{$user_phone}', '{$user_tour}', '{$user_email}', '{$hash}', '{$city_id}', '{$cat_id}', '{$user_photo}')") or FUNCTIONS::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);
    ...
    public function EXPORT_PLACES_LIST(){
        ...
        `tour_requests`.`id` as place_rating , `tour_requests`.`id` as place_reviews, `tour_requests`.`city_id` as place_city
        FROM `tour_requests`
        WHERE
        tour_requests.`city_id` = '{$get_cid}'
        AND
        tour_requests.`dept_id` = '{$get_did}'
...
SELECT `tour_requests`.`id` as place_id , `tour_requests`.`place_name` , `tour_requests`.`phone_number` AS `place_contact_number` ,
        `tour_requests`.`place_link` AS `place_tour_link` , `tour_requests`.`place_image` ,
        `tour_requests`.`latitude` AS place_latitude,
        `tour_requests`.`longitude` AS place_longitude,
        `tour_requests`.`place_info`,
        `tour_requests`.`city_id` AS place_city,
        `tour_requests`.`id` as place_rating , `tour_requests`.`id` as place_reviews
        FROM `tour_requests`
        LEFT JOIN `members` ON tour_requests.account_id = members.id
        WHERE
        (members.`city` = '{$get_cid}' OR tour_requests.`city_id` = '{$get_cid}')
        AND
        (members.`dept_id` = '{$get_did}' OR tour_requests.`dept_id` = '{$get_did}')
        AND tour_requests.`approved` = '1'") or FUNCTIONS::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);
        ...
        while ($row = mysql_fetch_assoc($result)) {
            ...
            foreach ($row as $col => $val) {
                ...
                ##    $val = $g[1];
                ##}
                ##}elseif($col == 'place_city' and !empty($val)){
                ##    $g = mysql_result( mysql_query("SELECT d_name_".$lang." FROM `departments` WHERE `id` = '{$val}'"), 0);
                ##    $val = $g;
                if($col == 'place_reviews'){
                    $g = @mysql_result( mysql_query("SELECT count( id ) FROM `reviews` WHERE `place_id` = '{$val}'"), 0);
                    $val = ($g > 0) ? $g : 0;
                }elseif($col == 'place_rating'){
                    $g = @mysql_result( mysql_query("SELECT review_rating FROM `reviews` WHERE `place_id` = '{$val}'"), 0);
                    $val = ($g > 0) ? $g : 0;
                }elseif($col == 'place_city'){
                    $g = @mysql_result( mysql_query("SELECT d_name_ar FROM `departments` WHERE `d_type` = 'city' and `id` = '{$val}'"), 0);
                    $val = $g;
                }
