...
var SKY = SKY || {};
SKY.Utils = {touchSupport: null, canvasSupport: null, transformation2dSupport: null, transformation3dSupport: null, transitionSupport: null, prefixedProps: [], hasTouchSupport: function () {
    ...
    return{r: parseInt(b[0], 16), g: parseInt(b[1], 16), b: parseInt(b[2], 16)}
}};
...
SKY.ImageLoader.prototype = {subscribe: function (a) {
...
SKY.Timer.prototype = {subscribe: function (a) {
    this.subscribers.push(a)
}, unsubscribe: function (a) {
    for (var b = 0; b < this.subscribers.length; b++)this.subscribers[b] === a && delete this.subscribers[b]
}, publish: function (a) {
...
SKY.CarouselItem.prototype = {init: function () {
    if (!1 == $.support.leadingWhitespace) {
        var a = 2 * parseInt(this.imageElement.css("padding-left")), b = 2 * parseInt(this.imageElement.css("border-left-width")), c = 2 * parseInt(this.element.css("padding-left")), d = 2 * parseInt(this.element.css("border-left-width"));
    ...
    this.carousel.settings.reflectionVisible && (this.reflection = SKY.CarouselItem.createReflection(this.imageElement,
        a.reflectionSize, a.reflectionAlpha), this.reflection.css({"float": "left", clear: "both", "margin-top": a.reflectionDistance + "px"}), this.element.append(this.reflection), this.update());
    this.loaded = !0;
    ...
    if (SKY.Utils.has2dTransformationSupport())a =
        "translate(" + this.x + "px, " + this.y + "px) scale(" + this.scale + ")", SKY.Utils.has3dTransformationSupport() && (a += " translateZ(0)"), this.element.css(SKY.Utils.getPrefixedProperty("transform"), a), this.element.css("opacity", this.alpha); else {
        var b = this.actualWidth * this.scale, c = this.actualHeight * this.scale;
        this.scaledX = this.x + (this.actualWidth - b) / 2;
        this.scaledY = this.y + (this.actualHeight - c) / 2;
        this.width = b;
        this.height = c;
        b = {left: this.scaledX, top: this.scaledY, width: this.width - this.extraItemSize, height: this.height -
            this.extraItemSize};
        c = {width: b.width - this.extraImageSize, height: b.height - this.extraImageSize};
        a.reflectionVisible && !SKY.Utils.hasCanvasSupport() ? (c.opacity = this.alpha, this.reflection && this.reflection.css({width: b.width, height: b.height, filter: SKY.CarouselItem.getAlphaFilterStatement(a.reflectionAlpha, a.reflectionSize, a.itemHeight)})) : b.opacity = this.alpha;
        this.element.css(b);
...
SKY.CarouselItem.createReflection = function (a, b, c) {
    var d = a.width(), g = a.height(), e = null;
    SKY.Utils.hasCanvasSupport() ? (e = $("<canvas>"), ctx = e.get(0).getContext("2d"), e.attr({width: d, height: b}), e.addClass("reflection"), ctx.save(), ctx.translate(0, g), ctx.scale(1, -1), ctx.drawImage(a.get(0), 0, 0, d, g), ctx.restore(), ctx.globalCompositeOperation = "destination-out", a = ctx.createLinearGradient(0, 0, 0, b), a.addColorStop(0, "rgba(0, 0, 0, " + (1 - c) + ")"), a.addColorStop(1, "rgba(0, 0, 0, 1.0)"), ctx.fillStyle = a, ctx.fillRect(0,
        0, d, b)) : (e = $("<img>"), e.attr({src: a.get(0).src}), e.css("filter", SKY.CarouselItem.getAlphaFilterStatement(c, b, g)));
    return e
};
...
SKY.Container.prototype = {setX: function (a, b) {
    this.x = a;
    this.update(b)
}, getLeft: function () {
    return this.element.position().left
}, setTopMargin: function (a) {
    var b = this.carousel.settings, c = b.itemHeight;
    ...
    }), SKY.Utils.hasTransitionSupport() ? (this.element.css(SKY.Utils.getPrefixedProperty("transition"), "left " + a + "s ease-out"), this.element.css("left", this.x)) : this.element.stop().animate({left: this.x}, 1E3 * a, function () {
        b.carousel.onSelectionAnimationEnd()
    })) : (SKY.Utils.hasTransitionSupport() && this.element.css(SKY.Utils.getPrefixedProperty("transition"), ""), this.element.stop().css({left: this.x}))
}};
SKY.Carousel = function (a, b) {
    this.settings = {itemWidth: 300, itemHeight: 450, distance: 15, startIndex: "auto", enableKeyboard: !0, enableMouseWheel: !0, reverseMouseWheel: !1, autoSlideshow: !1, autoSlideshowDelay: 2.5, loop: !0, selectedItemDistance: 50, selectedItemZoomFactor: 1, unselectedItemZoomFactor: 0.6, unselectedItemAlpha: 0.6, motionStartDistance: 150, topMargin: 30, preload: !0, showPreloader: !0, navigationButtonsVisible: !0, gradientStartPoint: 0.15, gradientEndPoint: 1, gradientOverlayVisible: !0, gradientOverlayColor: "#fff", gradientOverlaySize: 215,
        reflectionVisible: !1, reflectionDistance: 4, reflectionSize: 100, reflectionAlpha: 0.38, slideSpeed: 0.45, selectByClick: !1, showContent: !1};
...
SKY.Carousel.prototype = {init: function () {
    this.initDOM();
    ...
    a.preload && !1 != $.support.leadingWhitespace && (this.dom.wrapper.css({visibility: "hidden", opacity: 0}), a.showPreloader && (this.preloader = $('<div class="sc-preloader"></div>'), this.dom.carousel.append(this.preloader)));
    ...
    var a = this.settings, b = parseInt(this.dom.items.css("padding-left")), c = parseInt(this.dom.items.css("border-left-width")), d = parseInt(this.dom.images.css("padding-left")), g = parseInt(this.dom.images.css("border-left-width"));
    ...
    if (a.gradientOverlayVisible) {
        var b = this.createGradientOverlay("left", a.gradientStartPoint, a.gradientEndPoint,
            a.gradientOverlayColor, a.gradientOverlaySize), a = this.createGradientOverlay("right", a.gradientStartPoint, a.gradientEndPoint, a.gradientOverlayColor, a.gradientOverlaySize);
    ...
    if(this.settings.showContent == !1){
    a.append(this.contentContainer);
    }

    !1 != $.support.leadingWhitespace &&
        this.settings.preload && this.contentContainer.css({visibility: "hidden", opacity: 0});
    this.dom.carousel.append(a)
}, initNavigationButtons: function () {
    var a = this;
    if (this.settings.navigationButtonsVisible) {
        var b = $('<a href="#" class="sc-nav-button sc-prev sc-no-select"></a>'), c = $('<a href="#" class="sc-nav-button sc-next sc-no-select"></a>');
        this.dom.carousel.append(b);
    ...
    var a = this, b = this.settings, c = this.dom.carousel.get(0);
    ...
    this.dom.carousel.trigger({type: "closestItemChanged.sc", item: a})
    ...
    var c = this.settings;
    if ("number" === typeof a)var d = this.carouselItems[a]; else"object" === typeof a && (d = a);
    ...
    this.dom.carousel.trigger({type: "itemSelected.sc", item: this.selectedItem})
    ...
    for (var a = this, b = this.settings, c = this.container,
        ...
        var f = this.carouselItems[e], h = d + f.x + b.itemWidth / 2 - this.centerX, k = Math.abs(h);
        if (k <= b.motionStartDistance)var p = this.calculateAlpha(k), l = this.calculateScale(k), n = this.calculateExtraDistance(k); else p = b.unselectedItemAlpha, l = b.unselectedItemZoomFactor, n = 0;
    ...
    function b(a) {
        var b = a.originalEvent, c = SKY.Utils.hasTouchSupport() ? b.touches[0].clientX : a.clientX, g = SKY.Utils.hasTouchSupport() ? b.touches[0].clientY : a.clientY, r = (c - startX) / 2 + n;
        m = f - c;
        f = c;
    ...
    function c(a) {
        var f = a.originalEvent, e = SKY.Utils.hasTouchSupport() ? f.changedTouches[0].clientX : a.clientX;
        a = SKY.Utils.hasTouchSupport() ? f.changedTouches[0].clientY : a.clientY;
        var f = Math.round(m / 20), h = d.closestItem.index();
    ...
    if ("A" != a.target.nodeName) {
        var d = this, g = this.settings, e = a.originalEvent, f = startX = SKY.Utils.hasTouchSupport() ? e.targetTouches[0].clientX : a.clientX, h = SKY.Utils.hasTouchSupport() ? e.targetTouches[0].clientY : a.clientY, k = this.carouselItems.length, p = d.centerX - (g.selectedItemDistance + g.itemWidth / 2), l = p - (g.itemWidth * g.unselectedItemZoomFactor + g.distance) * (k - 1), n = this.container.getLeft(), m = 0, t = !1, q = !1, u = !1;
        this.timer && this.timer.stop();
    ...
    this.dom.carousel.trigger({type: "selectionAnimationStart.sc", item: this.selectedItem})
}, onSelectionAnimationEnd: function () {
    this.dom.carousel.trigger({type: "selectionAnimationEnd.sc", item: this.selectedItem})
...
window.Modernizr = function (a, b, c) {
    ...
    function e(a, b, f) {
        var e = a.charAt(0).toUpperCase() + a.slice(1), h = (a + " " + n.join(e + " ") + e).split(" ");
        if (d(b, "string") || d(b, "undefined"))b = g(h, b); else {
    ...
    var f = {}, h = b.documentElement, k =
        b.createElement("modernizr"), p = k.style, l = " -webkit- -moz- -o- -ms- ".split(" "), n = ["Webkit", "Moz", "O", "ms"], m = ["webkit", "moz", "o", "ms"], k = {}, t = [], q = t.slice, u, w = function (a, c, d, e) {
        var f, g, k, p, l = b.createElement("div"), n = b.body, m = n || b.createElement("body");
    ...
    Function.prototype.bind || (Function.prototype.bind = function (a) {
        ...
        var c = q.call(arguments, 1), d = function () {
            if (this instanceof
                ...
                var e = new e, f = b.apply(e, c.concat(q.call(arguments)));
    ...
    k.touch = function () {
        ...
        return"ontouchstart"in a || a.DocumentTouch && b instanceof DocumentTouch ? c = !0 : w(["@media (",
            l.join("touch-enabled),("), "modernizr){#modernizr{top:9px;position:absolute}}"].join(""), function (a) {
            c = 9 === a.offsetTop
        }), c
    };
    ...
    f.addTest = function (a, b) {
        if ("object" == typeof a)for (var d in a)v(a, d) && f.addTest(d, a[d]); else {
            a = a.toLowerCase();
            if (f[a] !== c)return f;
