	...
	$.fn.scrollAgenda = function(options)
	{
		...
		this.each(function(i)
		{
			...
			columns.children('.agenda-header').each(function(i)
			{
				...
				{
					$('<a href="#" title="'+settings.prevTitle+'" class="agenda-previous"><span class="icon-right-round"></span></a>').prependTo(header).applySetup();
				}

				// Next
				if (i < nbColumns-1 && header.children('.agenda-next').length === 0)
				{
					$('<a href="#" title="'+settings.nextTitle+'" class="agenda-next"><span class="icon-left-round"></span></a>').prependTo(header).applySetup();
				}
			});
			...
			setFirstColumn = function(firstCol, animate)
			{
				...
				firstCol.addClass('agenda-visible-column agenda-visible-first').stop(true)[animate ? 'animate' : 'css']({
					right:	'0%',
					left:	(100-colSize)+'%',
					marginRight: '-1px'			// This is to hide right border
				},
