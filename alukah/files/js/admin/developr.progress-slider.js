	...
	function _parseMarksOption(value, label, labelAlign, min, max, useExtremes, insetExtremes, horizontal)
	{
		...
		for (i = 0; i < value.length; ++i)
		{
			...
			if (value[i].align == undefined)
			{
				...
				{
					value[i].align = horizontal ? 'left' : 'bottom';
				}
				else if (insetExtremes && value[i].value === max)
				{
					value[i].align = horizontal ? 'right' : 'top';
				}
				else
				{
	...
	$.fn.buildTrack = function(options)
	{
		this.each(function()
		{
			...
			if (!track[0].style.width || !track[0].style.width.match(/[0-9\.]+(px|em)/i))
			{
				track.sizechange(function()
				{
					...
					hMarks.each(function(i)
					{
						...
						element.css('right', Math.round((value-settings.min)/(settings.max-settings.min)*(size-1)+1)+'px');
					});
			...
			for (i = 0; i < innerMarks.length; ++i)
			{
				...
				iMarks = iMarks.add($('<span class="inner-mark" style="'+(horizontal ? 'right' : 'bottom')+': '+position+'px"></span>').appendTo(track).data('mark-value', innerMarks[i].value));
			}
			for (i = 0; i < topMarks.length; ++i)
			{
				...
				tMarks = tMarks.add($('<span class="top-mark" style="right: '+position+'px">'+label+'</span>').appendTo(track).data('mark-value', topMarks[i].value));
			}
			...
			for (i = 0; i < bottomMarks.length; ++i)
			{
				...
				bMarks = bMarks.add($('<span class="bottom-mark" style="right: '+position+'px">'+label+'</span>').appendTo(track).data('mark-value', bottomMarks[i].value));
			}
		...
		this.each(function()
		{
			...
			function updateCursor(cursor, value, finalValue)
			{
				...
				if (horizontal)
				{
					cursor.stop(true)[animated ? 'animate' : 'css']({
						right: Math.round((value-settings.min)/(settings.max-settings.min)*(size-cursor.outerWidth(true)))+'px'
				...
				if (useTooltips)
				{
					...
					{
						tooltip.css('margin-right', -(Math.round(tooltip.outerWidth()/2)-1)+'px');
					}
				}
				else
				{
			...
			function updateBar(start, end)
			{
				...
				if (horizontal)
				{
					bar.stop(true)[animated ? 'animate' : 'css']({
						right: Math.round((start-settings.min)/(settings.max-settings.min)*size)+'px',
						left: Math.round((settings.max-end)/(settings.max-settings.min)*size)+'px'
					}, animated ? settings.animateSpeed : null);
				}
				else
				{
			...
			cursors.on('touchstart mousedown', function(event)
			{
				...
				var element = $(this).addClass('dragging'),
					...
					initialPosition = horizontal ? element.parseCSSValue('right') : element.parseCSSValue('bottom'),
				...
				function watchMouse(event)
				{
					var availableSpace = size-(horizontal ? element.outerWidth(true) : element.outerHeight(true)),
						offsetHolder = touchEvent ? event.originalEvent.touches[0] : event,
						position = Math.max(0, Math.min(availableSpace, initialPosition+(horizontal ? mouseX-offsetHolder.pageX : mouseY-offsetHolder.pageY))),
