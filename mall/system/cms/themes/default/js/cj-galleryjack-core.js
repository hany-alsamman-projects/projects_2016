...
(function($) {
	...
	$.cjBgSlideshow = function(data, vData) {
		...
		cacheBust = $.browser.webkit,
		useCategory = catalog.length,
		isPhone = DetectTierIphone(),
		...
		function init() {
			...
			if($.browser.msie) {
				...
				var vrs = parseInt($.browser.version, 10);
		...
		$.cjBgSlideshow.setCategory = function(cat) {
			...
			if(!firstFade) {
				...
				if(fading === 0) {
					...
					// wipe transition
						...
						var winPush = winWide + offsetX;
						
						if(goingRight) {
							
							if(bgOn === 1) {
								
								imgOne.css({clip: "rect(0px, " + winPush + "px, " + winTall + "px, " + winPush + "px)", display: "block"}).animate(
								
									{"clip": "rect(0px, " + winPush + "px, " + winTall + "px, " + "0px)"}, 
									time, 
									afterTrans
								...
								imgTwo.css({clip: "rect(0px, " + winPush + "px, " + winTall + "px, " + winPush + "px)", display: "block"}).animate(
							...
							if(bgOn === 1) {
								...
								imgOne.css({clip: "rect(0px, 0px, " + winTall + "px, 0px)", display: "block"}).animate(
								...
								imgTwo.css({clip: "rect(0px, 0px, " + winTall + "px, 0px)", display: "block"}).animate(
