...
<?php
        ...
        if($this->uri->segment(2) == false) $myclass = 'current';

?>    
		<li id="home" class="home <?=$myclass?>"><?php echo anchor('admin', lang('cp_admin_home_title'), 'class="top-link no-submenu' . (!$this->module > '' ? ' current' : '').'"');?></li>
		...
		foreach ($menu_items as $menu_item)
		{
			...
			if ($count > 0)
			{
				...
				if ($count > 1 )
				{
                    ...
                    if($menu_item == 'design'){                                            
					   echo '<li class="design '.$class.'" title="design">';
                    
                    }elseif($menu_item == 'content'){                       
                       echo '<li class="content '.$class.'" title="content">';
                       
                    }else{
                        echo '<li class="'.strtolower($menu_item).' '.$class.'" title="'.$menu_item.'">';
                    }
				...
				if (array_key_exists($menu_item, $modules)) 
				{
					...
					foreach ($modules[$menu_item] as $module)
					{
						...
						{
							echo '<li class="'.strtolower($module['slug']).' '.$class.'">' . anchor('admin/'.$module['slug'], $module['name'], array('class'=>$class)) . '</li>';
						}
					}
				}
			}
		...
		<li class="settings"><?php echo anchor('admin/settings', lang('cp_nav_settings'), 'class="top-link no-submenu' . (($this->module == 'settings') ? ' current"' : '"'));?></li>
		<?php endif; ?>

		<?php if (array_key_exists('modules', $this->permissions) OR $this->user->group == 'admin'): ?>
		<li class="addons"><?php echo anchor('admin/modules', lang('cp_nav_addons'), 'class="last top-link no-submenu' . (($this->module == 'modules') ? ' current"' : '"'));?></li>
		<?php endif; ?>
