 ...
 * @author 		Hany alsamman and Yorick Peterse - PyroCMS Development Team
 * @package 	CodexFW
...
class Admin extends Admin_Controller
{
 	...
 	public function index()
	{
		...
		if (ENVIRONMENT == 'live' AND $this->settings->ga_email AND $this->settings->ga_password AND $this->settings->ga_profile)
		{
			// Not FALSE? Return it
			if ($cached_response = $this->pyrocache->get('analytics'))
			{
                ...
                $data['total_visits'] = $cached_response['total_visits'];
			}
			...
			else
			{
				try
				{
					...
					$this->pyrocache->write(array('analytic_visits' => $flot_data_visits, 'analytic_views' => $flot_data_views, 'total_visits' => $total_visits), 'analytics', 60 * 60 * 6); // 6 hours
				}
		...
		{
			$data['messages']['error'] = sprintf(lang('cp_google_analytics_missing'), anchor('admin/settings', lang('cp_nav_settings')));
		}
        

        // Check if backup installed or enabled
        if(InArray($this->module_m->get_all(NULL), 'slug', 'backup')){        
            // load backup language file
            $this->load->language('backup/backup');
            
            // load backup config file
            $this->load->config('backup/backup');        
            
            // load backup module
            $this->load->model('backup/backup_m');   
            
            $backup_info = $this->backup_m->index();
            
            $data['messages']['notice'] = lang('data_backup_dashboard').' '.lang('data_last_backup').' '.$backup_info['last_backup_date'];
        }
	 ...
	 *
	 * @author Jerel Unruh - Codex Corp "Experts" Dev Team
	 */
