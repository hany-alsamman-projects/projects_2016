...
class Admin extends Admin_Controller {
	...
	public function index()
	{
        ...
        $total_users = $this->users_m->count_by($base_where);
        
		// Create pagination links
		$pagination = create_pagination('admin/users/index', $total_users);
	...
	public function create()
	{
		...
		if ($this->form_validation->run() !== FALSE)
		{
			...
			if ($user_id = $this->ion_auth->register($username, $password, $email, $user_data, $group->name))
			{
                ...
                //$myuser = $this->ion_auth->get_user_by_email($email);
                
                $this->load->models(array('galleries/galleries_m','files/file_folders_m'));
 
                $this->file_folders_m->insert(
                array(
					'name'			=> $username,
					'slug'			=> $username,
					'parent_id'		=> 1,
                ...
                $myfolder = $this->file_folders_m->get_by('name', $username);

                $myalbum = array(
                    			'title'				=> $username,
                    			'slug'				=> $username,
                    			'folder_id'			=> $myfolder->id,
                                'user_id'			=> $user_id,
                    			'thumbnail_id'		=> NULL,
                    			'description'		=> NULL,
                ...
                //create galleries album
                $this->galleries_m->insert($myalbum);
	...
	public function edit($id = 0)
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
            ...
            // Hack to set profile fileds
            $update_data['bio'] = $this->input->post('bio');
            ## Address field
            $update_data['address_line1'] = $this->input->post('address_line1');
            ## Awards field
            $update_data['address_line2'] = $this->input->post('address_line2');
            ## Works field
            $update_data['address_line3'] = $this->input->post('address_line3');
            $update_data['phone'] = $this->input->post('phone');
            $update_data['mobile'] = $this->input->post('mobile');
            $update_data['msn_handle'] = $this->input->post('msn_handle');
            $update_data['twitter_access_token'] = $this->input->post('twitter_access_token');
            
            $update_data['user_power'] = $this->input->post('user_power');
            $update_data['gravatar'] = $this->input->post('gravatar');
		...
		$this->data->member = & $member;
		$this->template
                //->append_metadata(js('admin/ajaxfileupload.js'))
