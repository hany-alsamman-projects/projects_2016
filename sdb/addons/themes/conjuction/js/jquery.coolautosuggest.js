			...
			function(e){
				if(e.keyCode!=37 && e.keyCode!=38 && e.keyCode!=39 && e.keyCode!=40 && e.keyCode!=13){
					if($(this).val().length>=minChars){
						$.ajax({
							url:me.callBackUrl + encodeURIComponent ( $(this).val() ),
										...
										if(arr.length>0){
											for(i=0;i<arr.length;i++){
												...
												if(me.thumbnail==true){
													var style=' style="background-image:url(addons/themes/conjuction/img/no_avatar.jpg);"';
													if(arr[i].thumbnail!=undefined && arr[i].thumbnail!='no_avatar.jpg' && arr[i].thumbnail!=''){
														style=' style="background-image:url('+BASE_URI+ 'files/thumb/' + arr[i].thumbnail + '/50/50);"';
                                                        //style=' style="background-image:url(addons/modules/vcard/images/no_avatar.png);"';
													}
												...
												html+='<div id="' + suggestRow + (i+1) + '" class="' + cssClass + '"' + id_field + '>' + thumb + '<div class="suggestion_title">' + arr[i].data.replace(new RegExp('(' + me.textField.val() + ')', 'gi'), "<b>$1</b>") + '</div>' + desc + '</div>';
											}
