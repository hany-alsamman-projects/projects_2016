(function($) {
    $(function() {
        ...
        function do_sortable()
        {
            $('.faq-list tbody').sortable({
                ...
                stop: function(event, ui) {
                    order = new Array();
                    $('input[name=action_to\\[\\]]').each(function(index) {
                        var faq_id = $(this).val();
                        order[index] = faq_id;
                    });
                    $.post(BASE_URL + '/admin/faq/update_order', { order : order }, function(data, response, xhr) {
        ...
        {
            var url = BASE_URL + '/admin/faq';
            if(form_id == 'categories')
            {
                url = BASE_URL + '/admin/faq/categories';
            }
        ...
        function load_content(load_url)
        {
            $('#content').slideUp('normal', function() {
               $(this).load(load_url, function(data, response, xhr) {
					...
					init_ckeditor();
					$('textarea, input[type="text"], select').uniform();
                    do_sortable();
