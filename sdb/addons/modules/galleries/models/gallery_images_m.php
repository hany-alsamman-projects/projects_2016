...
class Gallery_images_m extends MY_Model
{
	 ...
	 *
	 * @author Jerel Unruh - CodexFW Dev Team
	 * @access public
	...
	public function get_gallery_by_uid($id)
	{
        
        $results = $this->db->get_where('galleries', array('user_id' => $id));
        return $results->row_object();		
	}
    
	public function get_profile_images_by_gallery($id)
	{
		// Grand finale, do what you gotta do!!
		$this->db
			// Select fields on files table
			->select('f.id as file_id, f.filename, g.id as gallery_id')
            // 
			->from('files f')
			// Set my gallery by id
			->where('g.id', $id)
