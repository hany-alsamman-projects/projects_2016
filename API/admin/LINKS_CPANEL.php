			...
			switch($type){
                ...
                case "AddPlace" :
                    ...
                    if(!empty($_POST['place_name']) && isset($_POST['sub_ok']) && $_POST['sub_ok'] == 1){
						...
						( `id`, `first_name`,  `place_name`, `place_link`,`place_info`, `account_id`, `place_image`, `city_id`,`dept_id`, `latitude`, `longitude`, `approved`, `campaign_place` , `days_left`, `tour_address` )
						VALUES
						(NULL, '{$first_name}', '{$place_name}', '{$place_link}','{$place_info}', '{$account_id}', '{$place_image}', '{$city_id}','{$dept_id}', '{$latitude}', '{$longitude}', '{$approved}', '{$campaign_place}', '{$days_left}', '{$tour_address}')") or parent::_debug(mysql_error(),__CLASS__,"MYSQL ERROR ON LINE ". __LINE__);
				...
				case "AddMember" :
                    ...
                    if($ok != 3){
                        ...
                        else
                        {
                            ...
                            ( `id`, `name`,`last_name`, `password`, `address`, `email`, `group_id`, `activated`, `dept_id`, `city`, `company_name`, `phone_number`, `fax_number`, `website`, `gmap` )
                            VALUES
                            (NULL, '{$account_name}', '{$account_last}', '{$account_pass}', '{$account_address}', '{$account_mail}', '{$account_group}', '{$activated}', '{$account_dept}', '{$account_city}', '{$company_name}', '{$phone_number}', '{$fax_number}', '{$website}', '{$gmap}')") or die(mysql_error());
