 ...
 * @modified by hany alsamman
 * @param string $user_login User's username for logging in
 * @param string $user_email User's email address to send password and add
 * @return int|WP_Error Either user's ID or error on failure.
 */
function register_new_user( $user_login, $user_email, $user_full_name, $user_mobile) {
	$errors = new WP_Error();
    ...
    if ( $user_full_name == '' ) {
        $errors->add( 'empty_full_name', __( '<strong>خطأ</strong>: يرجى التأكد من كتابة اسمك الكامل.' ) );
    }

    do_action( 'register_post', $sanitized_user_login, $user_full_name, $errors );

    $errors = apply_filters( 'registration_errors', $errors, $sanitized_user_login, $user_full_name );

    if ( $user_mobile == '' ) {
        $errors->add( 'empty_user_mobile', __( '<strong>خطأ</strong>: يرجى التأكد من ادخال رقم الموبايل.' ) );
    }

    do_action( 'register_post', $sanitized_user_login, $user_mobile, $errors );

    $errors = apply_filters( 'registration_errors', $errors, $sanitized_user_login, $user_mobile );
    ...
    add_user_meta( $user_id, 'last_name', $user_full_name ); //added by hany alsamman

    add_user_meta( $user_id, 'aim', $user_mobile ); //added by hany alsamman
...
case 'register' :
	...
	if ( $http_post ) {
        ...
        $user_full_name = $_POST['full_name']; // added by hany alsamman
        $user_mobile = $_POST['user_mobile']; // added by hany alsamman
		$errors = register_new_user($user_login, $user_email,$user_full_name, $user_mobile); //modified by hany alsamman
		if ( !is_wp_error($errors) ) {
