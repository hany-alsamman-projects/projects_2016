...
if ( have_posts() ) : $i = 0; ?>
	...
	<?php
        ...
        $full_name = ( get_the_author_meta( 'full_name', $post->post_author ) != false)? get_the_author_meta( 'full_name', $post->post_author ) : 'لايوجد';
		...
		<li style="direction: rtl; text-align: left" id="ticket-<?php the_ID(); ?>" <?php post_class( 'ticket ' . ( $i % 2 ? '' : 'alt ' ) . qc_taxonomy( 'ticket_status', 'slug' ) ); ?>>
			...
			<p class="ticket-author" style="float: left">
                ...
                if(qc_status_slug() == 'finish' && current_user_can('edit_published_posts')){
                    $myba = get_post_meta( get_the_ID(), '_balance' );

                    echo ' <span>قيمة المبلغ</span>  ';
                    echo '<strong style="font-size:12pt">'.$myba[0].'</strong>';
                }
                ?>
			</p>
