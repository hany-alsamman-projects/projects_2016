...
function cpm_page_read_check($thread_id, $user_id){
	...
	if($wpdb->get_var($query)==0){
		if(current_user_can('administrator')){
			...
			if($wpdb->get_var($query)==0){
				$cpm_errors->add('noAuth', __('الرسالة التي تحاول قرائتها قد تم حذفها', 'cubepm'));
			}
			else{
...
function cpm_page_read_buildInfo($thread_id, $user_id){
	...
	$html .= '<span class="cpm-thread-meta-label">' . __('المرسل اليه', 'cubepm') . ':</span> ';
	$from = array();
	...
	if($cpm_currentThread_isParticipant && get_option('cpm_email_enabled')){
		...
		$html .= '<label for="cpm_subscribe_value">' . __('اعلمني على بريدي بتنيه , عندما يتم الرد على هذه الرسالة', 'cubepm') . '</label>';
		$html .= '</form>';
...
function cpm_page_read_buildThread($thread_id, $user_id){
	...
	$html .= '<thead>';
	$html .= '<tr>';
	$html .= '<th class="cpm-thread-from">' . __('من', 'cubepm') . '</th>';
	$html .= '<th class="cpm-thread-message">' . __('الرسالة', 'cubepm') . '</th>';
	$html .= '</tr>';
	$html .= '</thead>';
	$html .= '<tbody>';
	$count = 0;
...
function cpm_page_read_buildReply($thread_id, $user_id, $cpm_errors = null){
	...
	$html .= '<h3>'. __('كتابة رد', 'cubepm') .'</h3>';
	if($cpm_errors != null){
		if(count($cpm_errors->get_error_messages())>0){
			$html .= cpm_htmlError($cpm_errors);
		}
		else{
			$html .= cpm_htmlMessage('تم ارسال البريد الداخلي الى المستلم بنجاح!', 'تم');
		}
	}
	...
	$html .= '<p><input type="submit" name="cpm_submit" id="cpm_submit" value="'.__('الرد', 'cubepm').' &raquo;" /></p>';
	$html .= '</form>';
	$html .= '</td></tr></tbody>';
	$html .= '</table>';
	return $html;
...
function cpm_page_read_process($thread_id){
	...
	if($message==''){
		$cpm_errors->add('emptyMessage', __('الرجاء تعبئة حقل الرسالة', 'cubepm'));
	}
