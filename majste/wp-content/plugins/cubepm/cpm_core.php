 ...
 * @return string
 */
function cpm_relativeTime($timestamp, $show_ending = true, $title_date_format = "j F Y, g:i a"){ 
	$difference = time() - $timestamp;
	$periods = array(__('sec','cubepm'), __('min','cubepm'), __('hour','cubepm'), __('day','cubepm'), __('week','cubepm'), __('month','cubepm'), __('year','cubepm'));
	$periods_plural = array(__('secs','cubepm'), __('mins','cubepm'), __('hours','cubepm'), __('days','cubepm'), __('weeks','cubepm'), __('months','cubepm'), __('years','cubepm'));
	...
	if($difference != 1 && $difference != 0) $periods[$j] = $periods_plural[$j];
	$text = "$difference $periods[$j] $ending";
	if($title_date_format){
		$text = '<span title="' . date($title_date_format, $timestamp + ( get_option( 'gmt_offset' ) * 3600 ) ) . '">' . $text . '</span>';
	}
	return $text;
}
