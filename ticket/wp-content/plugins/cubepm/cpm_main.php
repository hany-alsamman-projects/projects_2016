...
function cpm_shortcode($atts){
	...
	if(is_user_logged_in()){
		...
		$html .= '<p>' . __('يجب عليك الدخول لاستخدام البريد الداخلي', 'cubepm') . '<br /><a href="' . wp_login_url( site_url($_SERVER["REQUEST_URI"]) ) .  '">' . __('اضغط هنا للدخول', 'cubepm') . ' &raquo;</a></p>';
	}
	$html .= '</div>';
	return $html;
}
...
function cpm_htmlError($wp_error){
	...
	foreach($errors as $error){
		$html .= '<p><strong>' . __('خطا', 'cubepm') . ':</strong> ' . $error . '</p>';
	}
	$html .= '</div>';
	return $html;
}
...
function cpm_ajax_recipient(){
    ...
    $contributor_ids = get_contributor_ids();

    //some magic by codexc :)
    $users = $wpdb->get_results('SELECT * from ' . $wpdb->prefix . 'users WHERE ID NOT IN ( '.$contributor_ids.' ) and user_login LIKE \''.$_REQUEST['q'].'%\'' . $limit, ARRAY_A);
    $response = array();
 ...
 */
function cpm_header(){
	$html = '<div class="cpm-header" style="direction:rtl">';
	$html .= '<a class="cpm-button" href="' . cpm_buildURL(array('cpm_action'=>'inbox')) . '">' . __('صندوق الوارد', 'cubepm') . ' (' . cpm_inboxCount() . ')</a> ';
	if(cpm_currentUserCanStartThread()){
		$html .= '<a class="cpm-button" href="' . cpm_buildURL(array('cpm_action'=>'new')) . '">' . __('انساء رسالة', 'cubepm') . '</a> ';
	}
	if(current_user_can('administrator')){
		$html .= '<a class="cpm-button" href="' . cpm_buildURL(array('cpm_action'=>'admin-inbox')) . '">' . __('عرض كل الرسائل الداخلية في صندوق الوارد', 'cubepm') . '</a> ';
	}
	$html .= '</div>';
	return $html;
}
