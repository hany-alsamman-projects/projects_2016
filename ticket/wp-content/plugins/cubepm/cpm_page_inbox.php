...
function cpm_page_inbox(){
	$html = '<h2>' . __('صندوق الوارد', 'cubepm') . '</h2>';
	global $current_user;
	get_currentuserinfo();
	...
	if(count($thread_ids)>0){
		...
		$html .= '<thead>';
		$html .= '<tr>';
		$html .= '<th class="cpm-inbox-subject">' . __('العنوان', 'cubepm') . '</th>';
		$html .= '<th class="cpm-inbox-from">' . __('المرسل', 'cubepm') . '</th>';
		$html .= '<th class="cpm-inbox-received">' . __('المرسل اليه', 'cubepm') . '</th>';
		$html .= '</tr>';
		$html .= '</thead>';
		$html .= '<tbody>';
		$count = 0;
		foreach($thread_ids as $thread_id){
			...
			$html .= '<tr class="' . (($count%2==0)?'even':'odd') . ' ' .  ((cpm_userCheckOpenedThread($user_id, $thread_id))?'تم المشاهدة':'لم تشاهد بعد') .'">';
		...
		$html .= cpm_htmlMessage(__('ليس لديك اي رسالة داخلية خاصة في صندوق الوارد', 'cubepm'));
	}
	return $html;
}
