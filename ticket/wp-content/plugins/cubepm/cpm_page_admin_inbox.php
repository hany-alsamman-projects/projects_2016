...
function cpm_page_admin_inbox(){
	$html = '<h2>' . __('عرض كل الرسائل', 'cubepm') . '</h2>';
	...
	if(count($thread_ids)>0){
		...
		$html .= '<th class="cpm-inbox-from">' . __('المرسل اليه', 'cubepm') . '</th>';
		$html .= '<th class="cpm-inbox-received">' . __('تم ارسالها منذ', 'cubepm') . '</th>';
		$html .= '</tr>';
		$html .= '</thead>';
		$html .= '<tbody>';
		$count = 0;
		...
		$html .= cpm_htmlMessage(__('ليس لديك اي رسالة داخلية في صندوقك', 'cubepm'));
	}
	return $html;
}
