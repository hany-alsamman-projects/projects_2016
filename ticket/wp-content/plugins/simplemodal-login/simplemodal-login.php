	...
	<form name="loginform" id="loginform" action="%s" method="post">
		...
		<p>
			...
			<input type="password" name="pwd" class="user_pass input" value="" size="20" tabindex="20" /></label>
				...
				__('تسجيل دخول', 'simplemodal-login'),
				__('اسم المستخدم', 'simplemodal-login'),
				__('كلمة المرور', 'simplemodal-login')
			);
		...
		<p class="nav">',
			...
			if ($this->options['reset']) {
				$output .= sprintf('<a class="simplemodal-forgotpw" href="%s" title="%s">%s</a>',
					...
					__('استرجاع كلمة سر مفقودة', 'simplemodal-login'),
					__('هل نسيت كلمة السر ؟', 'simplemodal-login')
				);
			}
		...
		function login_js() {
			...
			wp_localize_script('simplemodal-login', 'SimpleModalLoginL10n', array(
				...
				'empty_username' => __('<strong>خطا</strong>: حقل اسم المستخدم فارغ.', 'simplemodal-login'),
				'empty_full_name' => __('<strong>خطا</strong>: حقل الاسم الثلاثي فارغ.', 'simplemodal-login'),
				'empty_password' => __('<strong>خطا</strong>: حقل كلمة المرور فارغ.', 'simplemodal-login'),
				'empty_email' => __('<strong>خطا</strong>: حقل البريد الالكتروني فارغ.', 'simplemodal-login'),
				'empty_all' => __('<strong>خطا</strong>: جميع الحقول مطلوبة.', 'simplemodal-login')
			));
...
<form name="registerform" id="registerform" action="%s" method="post">
	...
	<p>
		<label>%s<br />
		<input type="text" name="full_name" class="user_login input" value="" size="20" tabindex="10" /></label>
	</p>
	<p>
		<label>%s<br />
		<input type="text" name="user_mobile" class="user_login input" value="" size="20" tabindex="10" /></label>
	</p>
	<p>
		<label>%s<br />
		<input type="text" name="user_email" class="user_email input" value="" size="25" tabindex="20" /></label>
				...
				__('التسجيل', 'simplemodal-login'),
				__('اسم المستخدم الذي سوف يستعمل لتسجيل الدخول', 'simplemodal-login'),
                __('الاسم الثلاثي', 'simplemodal-login'),
                __('رقم الجوال', 'simplemodal-login'),
				__('البريد الالكتروني', 'simplemodal-login')
			);
	...
	<p class="nav">
		<a class="simplemodal-login" href="%s">%s</a>',
				__('كلمة السر سوف تصلك بعد التسجيل', 'simplemodal-login'),
				__('تسجيل', 'simplemodal-login'),
				__('الغاء', 'simplemodal-login'),
	...
	<form name="lostpasswordform" id="lostpasswordform" action="%s" method="post">
		...
		<p>
			...
			<input type="text" name="user_login" class="user_login input" value="" size="20" tabindex="10" /></label>
				...
				__('استعادة كلمة السر', 'simplemodal-login'),
				__('اسم المستخدم او البريد:', 'simplemodal-login')
			);
		...
		<p class="nav">
			<a class="simplemodal-login" href="%s">%s</a>',
				__('احصل على كلمة سر جديدة', 'simplemodal-login'),
				__('الغاء', 'simplemodal-login'),
				site_url('wp-login.php', 'login'),
				__('تسجيل دخول', 'simplemodal-login')
			);
			...
			if ($this->users_can_register && $this->options['registration']) {
				$output .= sprintf('| <a class="simplemodal-register" href="%s">%s</a>', site_url('wp-login.php?action=register', 'login'), __('تسجيل', 'simplemodal-login'));
			}
