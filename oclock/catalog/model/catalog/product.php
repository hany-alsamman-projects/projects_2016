...
class ModelCatalogProduct extends Model {
	...
	public function getBarOfProduct($category_id) {
       ...
       $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE category_id = '" . (int)$category_id . "'");
		...
		foreach ($query->rows as $result) {
			...
			if(isset($result['product_id']) && $result['product_id'] > 0){
                $getpro = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$result['product_id'] . "'");

                //print $getpro->row['image'];
            }
		}        
		return $getpro->rows;
	}
