...
<?php
			...
			$c = mysql_query("SELECT * FROM `en_products` ORDER BY id DESC");
			
			$i=0; 
            	
			while($row = mysql_fetch_object($c)){
                
                $dept_name = mysql_result( mysql_query("SELECT en_d_name FROM `departments` WHERE `id` = '{$row->in_dept}'") , 0) or die(mysql_error());
...
print <<<EOF
                                        $lastline_start
											<div class="item">
												<div class="inner">
													<a class="thumb" href="#"><img width="100" height="125" src="$thumbnail" alt=""></a>
													<div class="data">
														<h4><a target="_blank" href="$row->url">$row->prodcut_name</a></h4>
														<p>$row->prodcut_details</p>
														<div class="meta">$added_time</div>
														<ul class="actions">
                                                        
															<li><a class="ico" href="javascript:showdiv($row->id)" title="edit"><img src="images/admin/pencil.png" alt="edit"></a></li>
															<li><a class="ico" href="index.php?section=DeleteItem&id=$row->id" title="Remove"><img src="images/admin/delete.png" alt="delete"></a></li>
														</ul>
													</div>
												</div>
											</div><!-- end item -->
