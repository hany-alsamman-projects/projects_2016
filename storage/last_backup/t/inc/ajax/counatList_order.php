...
if(ceckAjax('l')){
    ...
    foreach($fileds as $key){
           ...
           if($key == 'item'){
               ...
               $html .= '<div style="position: relative; height:50px" ><input style="width: 50px" class="check_order_quantity" id="'.$id.'_'.$price.'" name="'.$key.'[]" ></div>';

           }elseif($key == 'price'){

               $price = mysql_result(mysql_query("select price from `items` WHERE `id` = '{$id}' "), 0);

               $html .= '<div style="position: relative; height:50px" ><input style="width: 50px" name="'.$key.'[]" value="'.$price.'" ></div>';
