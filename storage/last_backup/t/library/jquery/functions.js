...
$.fn.loadfns = function(specificfns) {
    ...
    $(".cus_info_area #save_note").live('click', function(e) {
        e.preventDefault();
        var area_val = $(".note_area").val();
        var customer_id = $(".note_area").attr('id');

        /* Send the data using post */
        var posting = $.post('inc/ajax/saveCusNote.php',{id:customer_id , note:area_val}).done(function( data ) {
            toastr.options.positionClass = 'toast-top-right';
            toastr.success('customer note has beed saved!');
            return false
        });
    });

    $(".cus_info_area #view_note_invoice").live('click', function(e) {
        e.preventDefault();
        var customer_id = $(".note_area").attr('id');

        /* Send the data using post */
        var posting = $.post('inc/ajax/last_invoice.php',{id:customer_id}).done(function( data ) {
            //jQuery(document).trigger('close.facebox');
            jQuery.facebox(data);
        });
    });

    $(".customer_row").live('mousedown', function(e) {
	...
	$(document)[0].oncontextmenu = function() {return false;}
        ...
        e.preventDefault();
        if( (e.which == 3) ) {
            //var get_id = $(this).parents(".customer_row").attr("id").split('_'); // id = get_id[1]
            var get_id = $(this).attr("id").split('_'); // id = get_id[1]
            jQuery.facebox({ ajax: 'inc/ajax/loadCusInfo.php?id='+get_id[1] });
        }
    });

    $.post('inc/ajax/loadItems.php',{op:'customers'}, function(data) {
        $('#items_board').html(data);
    });

    $.post('inc/ajax/loadCus.php',{}, function(data) {
        $('#customers_board').html(data);
    });

    // refersh page every 2 seconds
    setInterval(function () {

        $.post('inc/ajax/check_update.php',{op:'items'}, function(data) {
            if(data == 'YES'){
        ...
        $.post('inc/ajax/check_update.php',{op:'customers'}, function(data) {
            if(data == 'YES'){
                $.post('inc/ajax/loadCus.php',{}, function(data) {
                    $('#customers_board').html(data);
                });
    ...
    $(".clear_area").live('click', function(e) {
        e.preventDefault();
        $( "#work_area .box-body").empty();
    });

    $("#multi_add").live('click', function(e) {
        $("#work_area .box-body").empty();
        e.preventDefault();
        var get_page = $.post('inc/ajax/add_forms.php').done(function( data ) {
    ...
    $("#quick_additem, #quick_addcustomer, #quick_addsupplier").live("submit", function(e) {
        e.preventDefault();
        var get_data = $(this).serialize();
        $(this).trigger('reset');
        var get_page = $.post('inc/ajax/multi_operation.php',get_data).done(function( data ) {
            /* Put the results in a div */
            if(data!=false){
                toastr.options.positionClass = 'toast-top-right';
                toastr.success('the submitted data added successfully!');
                return false;
            }else{
                toastr.options.positionClass = 'toast-top-right';
                toastr.error('there wrong with the submitted data!');
                return false;
            }
        });
    });
    ...
    $('.customer_row').live('click', function(e) {
        ...
        if ($('#sort-boxes #process_type').val() == '2') {
            toastr.options.positionClass = 'toast-top-right';
            toastr.warning('hey! why try need to add customer?');
            return false;
        }
        ...
        if($(this).css('background-color') == 'transparent'){
            $(".customer_row").css('background-color','transparent');
        ...
        //add_cus(<?=$r['id']?>,'<?=$r['company']?>',this)
        //alert( $('#smp-form #box_customer_id').val() );

    });

    // Main function to check what the process type
    $('.item_row').live('click', function(e) {
        e.preventDefault();
        var $this = this;
        var process_type = $('#sort-boxes #process_type').val();
        ...
        //edit invoice
        if(process_type == '3'){
            $(this).edit_invoice($this);
        }

        //edit order
        if(process_type == '4'){
            $(this).edit_order($this);
        }


        $('#smp-form button').removeAttr("disabled");
    });
