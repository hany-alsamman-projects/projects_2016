$(function () {
    ...
    $('.close_invoice').live('click', function (e) {
        e.preventDefault();
        close_invoice();
    });
    ...
    $(".check_drop_changes").live('change', function () {
        var get_parent = $(this).closest('tr').attr('id'); // get id only

        var get_value = this.id.split('_'); // its have item id_ price
        var get_id = get_value[1]; // get id only
        $("#"+get_parent+" #qun_" + get_id).find(".check_quantity").val(''); // its have item id _ price
    });
    ...
    function DeepCheck(target) {
        ...
        $('#'+target+' table tbody tr').each(function () {
            ...
            var get_parent = $(this).attr('id');

            var fetch = get_parent.split('_');

            var row_id = fetch[1]; // get id only

            var row_value = $("#"+get_parent+" #qun_" + row_id).find(".check_quantity").val(); // its have item id _ price
            var getindex = $('#'+get_parent+' #order_' + row_id + ' :selected').text().match(/\[(.*?)\]/);

                $.ajax({
                    type: "POST",
                    cache: false,
                    data: { quantity: row_value, item_id: row_id,lots: getindex[1] },
                    url: 'inc/ajax/check_qunt.php',
                    async: false,
                    success: function (data) {

                        if (data == 0) {
                            $('#' + get_parent).css('background-color', "#ff7a7a");
                            //$('.invoice_form button').attr("disabled", "true"); //disable invoice save buttons
                            result.push(row_id);
                        }
                    }
    ...
    $('#save_invoice, #save_print, #save_customized').live('click', function (e) {
        ...
        var validation = $('#new-invoice-form').validationEngine('validate');

        var checkErrors = DeepCheck('new-invoice-form');

        if (validation == 1 && checkErrors == 0) {
    ...
    $(".check_price_changes").live('keyup', function () {
        ...
        var row_id = $("#"+get_parent+" #qun_" + id).find(".check_quantity").attr('id'); // its have item id _ price
        var row_value = $("#"+get_parent+" #qun_" + id).find(".check_quantity").val(); // its have item id _ price
    ...
    $(".check_quantity").live('keyup', function () {
        ...
        var getindex = $('#'+get_parent).find('#order_' + getval_id + ' :selected').text().match(/\[(.*?)\]/);

        /* Send the data using post */
        var posting = $.post("inc/ajax/check_qunt.php", {
            quantity: row_value,
            item_id: getval_id,
            lots: getindex[1]
        });
        ...
        posting.done(function (data) {
            if (data == 0) {
                toastr.options.positionClass = 'toast-top-right';
                toastr.error('quantity not available in this container!');
                $('#'+get_parent).find('#' + row_id).val(''); //reset
                $('#' + get_parent).css('background-color', "#ff7a7a");
                return false;

            } else {

                $('#' + get_parent).css('background-color', "");
    ...
    function update_total_bill(sum) {
        ...
        $('#t_invoice table tbody tr').each(function () {
    ...
    $.fn.add_invoice = function (th) {
        ...
        /* Send the data using post */
        var posting = $.post("inc/ajax/counatList.php", {
            id: get_item_id[2],
            c: get_customer_id
        });
        /* Put the results in a div */
    ...
    $('.restart_item').live('click', function (e) {
        e.preventDefault();

        var get_parent = $(this).closest('tr').attr('id').split('_');
        var row_id = get_parent[1] // get id only
        var opr_id = $(this).attr('rel');

        $(this).closest('tr').remove();

        /* Send the data using post */
        var posting = $.post('inc/ajax/delItem.php', {
            opr_id: opr_id
        }).done(function (data) {
    ...
    $('.delete_invoice').live('click', function (e) {
        e.preventDefault();
        /* Send the data using post */
        var posting = $.post("inc/ajax/delInvoices.php", {
            id: this.id
        }, data);
        /* Put the results in a div */
        posting.done(function (data) {
    ...
    $(".fetch_included_invoice").live('click', function (e) {
        ...
        /* Send the data using post */
        var posting = $.post('inc/ajax/veiwInvoice.php', {
            id: invoice_id
        }).done(function (data) {
    ...
    $('.save_modified_invoice, .save_modified_print').live('click', function (e) {
        e.preventDefault();
        ...
        var validation = $('#edit-invoice-form').validationEngine('validate');

        var checkErrors = DeepCheck('edit-invoice-form');
