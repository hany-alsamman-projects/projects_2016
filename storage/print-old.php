...
if($rows>0){
	...
	$sql2="select sum(v.qunt) as q ,avg(v.price) as p,i.name 
...
<div style="margin:15px; font-size:16px;">Invice No: <?=$id?><br/>Customer: <?=$company?></div>
...
NAME: <strong><?=$company?></strong><br /><br />
...
<div style="font-size:14px; margin-bottom:10px"><?=nl2br(get_val('inv_text','footer',1))?></div>
  <div style="font-size:13px">Invice Date : <?=$date?></div>
