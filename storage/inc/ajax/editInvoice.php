...
function create_invoice_row($item_id,$inv_id,$c,$opr_id,$ord_id){
    ...
    foreach($fileds as $key){
        ...
        if($key == 'item'){
            ...
            $html .= '<div style="position: relative" ><select style="width:100px; margin-bottom: 20px" id="order_'.$id.'" name="order_id[]" class="drop select validate[required]">';
            ...
            while($i<$rows){
                ...
                $opr_ord_id =mysql_result($res,$i,'o.ord_id');
                $name=mysql_result($res,$i,'o.name');
                $q=chekQunt($c_id,$id,0,1);
                if($ord_id==$c_id){
                    $html .= '<option value="'.$c_id.'" selected="true"><b>#'.$name.'</b> ['.$q.']</option>';
                }else{
                    $html .= '<option value="'.$c_id.'"><b>#'.$name.'</b> ['.$q.']</option>';
                }
            ...
            $qunt = mysql_result(mysql_query("SELECT qunt FROM `inv_opr` WHERE `id` = '{$opr_id}' and `inv_id` = '{$inv_id}' and `it_id` = '{$id}' "), 0);

            $html .= '<div style="position: relative; height:50px" ><input autocomplete="off" style="position: absolute; top:0; width:55px" class="check_quantity validate[required, custom[onlyNumber]]" value="'.$qunt.'" id="'.$id.'" name="'.$key.'[]" ></div>';

        }elseif($key == 'price'){

            $price = mysql_result(mysql_query("SELECT price from `inv_opr` WHERE `id` = '{$opr_id}' and `inv_id` = '{$inv_id}' "), 0);

            $html .= '<div style="position: relative; height:50px" ><input id="'.$id.'" class="check_price_changes validate[required]" style="width:55px" name="'.$key.'[]" value="'.$price.'" ></div>';
            ...
            $html .= '<a href="#" title="Restart item" id="item_'.$id.'"  rel="'.$opr_id.'" class="restart_item"><img width="17" title="Restart item" alt="Restart item" height="17" src="images/refresh.png"></a>';
                                    ...
                                    <div id="t_invoice">
                                        ...
                                        if($rows2>0){
                                            while($i<$rows2){
                                                ...
                                                create_invoice_row($it_id,$id,$c_id,$opr_id,$ord_id);
