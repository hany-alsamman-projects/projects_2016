...
function create_order_row($ord_id ,$item_id, $opr_id){
    ...
    foreach($fileds as $key){
        ...
        if($key == 'item'){
            ...
            $qun = mysql_result(mysql_query("select qunt from `ord_opr` WHERE `id` = '{$opr_id}'"), 0);
            $price = mysql_result(mysql_query("select price from `ord_opr` WHERE `id` = '{$opr_id}'"), 0);

            $html .= '<div style="position: relative; height:50px" ><input style="width: 60px" class="check_order_quantity validate[required, custom[onlyNumber]]" value="'.$qun.'" id="'.$id.'_'.$price.'" name="'.$key.'[]" ></div>';

        }elseif($key == 'price'){

            $price = mysql_result(mysql_query("select price from `ord_opr` WHERE `id` = '{$opr_id}'"), 0);

            $html .= '<div style="position: relative; height:50px" ><input id="'.$id.'" class="check_order_price_changes validate[required]" style="width:60px" name="'.$key.'[]" value="'.$price.'" ></div>';
...
if($rows>0){
    ...
    echo '<div id="smp-form" class="box g8 flt-l edit_order_form">
                <p class="box-ttl">Edit Order NO# '.$id.' <img src="images/close.png" alt="" class="close_edit_order flt-r"></p>
                <div class="box-body form">
                    ...
                    <form id="edit-order-form">
                        ...
                        <div id="box_customer_name" class="g16">
                            <input type="text" name="lot_no" placeholder="LOT#" value="'.$name.'" data-ttip="LOT #" class="validate[required] g3 ttip">
                            <input type="text" name="container_no" placeholder="Container number" data-ttip=Container number" value="'.$num.'" class="validate[required] g5 ttip">
                            <input type="text" name="expenses" placeholder="Expenses of the container" data-ttip="Expenses of the container" value="'.$expenses.'" class="validate[required, custom[onlyNumber]] g5 ttip">
                            <input type="text" name="po" placeholder="P.O" data-ttip="P.O" value="'.$po.'" class="validate[required] g2 ttip">
                        </div>
                        ...
                        <div id="box_customer_name" class="g16">
                            ...
                            <input type="text" name="shipping_company" data-ttip="shipping company" value="'.$shipping_company.'" placeholder="Shipping Company" class="validate[required] g5 ttip">
                            <input type="text" name="shipping_expenses" data-ttip="shipping expenses" value="'.$shipping_expenses.'" placeholder="Shipping Expenses" class="validate[required, custom[onlyNumber]] g5 ttip">
                        </div>
    ...
    $sql2="select * from ord_opr ord , items i , orders o  where ord.ord_id='$id' and ord.it_id=i.id and ord.ord_id=o.id ORDER BY ord.id ASC";
    ...
    if($rows2>0){
        ...
        while($i<$rows2){
            ...
            create_order_row($id, $item_id, $v_id);
